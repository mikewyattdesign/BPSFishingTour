jQuery.fn.fadeSlideShow = function(options) {
	return this.each(function(){
		settings = jQuery.extend({
			...
			NextElement: 'nextSlide', // default id for next button
			NextElementText: 'Next >', // default text for next button
			PrevElement: 'prevSlide', // default id for prev button
			PrevElementText: '< Prev', // default text for prev button
			ListElement: 'dotList', // default id for image / content controll list
			ListLi: 'dotLi', // default class for li's in the image / content controll
		...
		var jumpTo = function(newIndex){
			...
			if( newIndex >= ActSlide ){
				jQuery('> *:lt('+(newIndex+1)+')', dotThis).fadeIn(settings.speed);
			}else if(newIndex <= ActSlide){
				jQuery('> *:gt('+newIndex+')', dotThis).fadeOut(settings.speed);
			}
